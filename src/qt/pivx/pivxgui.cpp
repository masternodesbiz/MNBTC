...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("mnbtc client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("mnbtc"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("mnbtc"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
